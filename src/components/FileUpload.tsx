...
export default function FileUpload({
  ...
  const processSmallFile = async (
    ...
    const response = await fetch("/api/upload", {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        blobUrls: [{ url: blobUrl, filename, path: storagePath }],
        sessionId,
      }),
    });
      ...
      for (const line of decoder
        ...
        try {
          ...
          if (data.event === "status")
            updateStatus(data.message, data.progress || 50);
        } catch {
          continue;
        }
      }
  ...
  const processFiles = useCallback(
    async (filesToProcess: Map<string, UploadedFile>) => {
      if (processingRef.current) return;

      const pending = Array.from(filesToProcess.values()).filter(
        (f) => f.status === "pending",
      );
      ...
      try {
        for (const uploadedFile of pending) {
          ...
          try {
            ...
            const { url: blobUrl, path: storagePath } =
              await uploadToStorage(file);

            if (isLarge) {
              updateStatus("Large PDF - async background processing...", 10);

              const { sessionId: newSessionId, pages } = await processWithAsync(
                blobUrl,
                file.name,
                storagePath,
                updateStatus,
              ...
              const newSessionId = await processSmallFile(
                blobUrl,
                file.name,
                storagePath,
                updateStatus,
  ...
  useEffect(() => {
    const pending = Array.from(files.values()).filter(
      (f) => f.status === "pending",
    );
        ...
        <div className="flex flex-wrap gap-2 justify-center">
          {SUPPORTED_FORMATS.map((f) => (
            <span
              key={f.ext}
              className={cn(
                "text-xs px-2 py-0.5 rounded bg-slate-800",
                f.color,
              )}
            >
              {f.label}
